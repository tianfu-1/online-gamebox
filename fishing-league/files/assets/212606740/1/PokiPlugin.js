var PokiPlugin = {
    ...
    init: function () {
        ...
        script.src = './scripts/v2/poki-sdk.js';
    ...
    onPauseWithPrevent : function(){
        this.onPause();
        this.disallowGameplayTimer = Date.now();
    },
    onPlayWithPrevent : function(){
        this.disallowGameplayTimer = -1;
        this.onPlay();
    },
    onPause: function () {
        if (this.isGamePaused) {
    ...
    onPlay: function () {
        if(Date.now() - this.disallowGameplayTimer < 4000){
            return;
        }
    ...
    showMidroll: function (callback) {
        ...
        if (typeof PokiSDK !== 'undefined') {
            ...
            pc.app.fire('FishingManager:PauseBonuses');
            PokiSDK.commercialBreak().
                then(function (success) {
                    pc.app.fire('FishingManager:ResumeBonuses');
                    if (callback) {
                        callback(success);
                    }
        ...
        PokiSDK.rewardedBreak().
            then(function (success) {
                ...
                if (options && options.disableEvents) {
                    ...
                    PokiPlugin.playFrames();
                }
            });

        PokiPlugin.totalGameplay = 0;
    },
    showRewardWithoutGameplay: function (callback, options) {
        if (PokiPlugin.adblock) {
            if (typeof pc !== 'undefined') {
                pc.app.fire('Overlay:Adblock');
            }
    ...
    showLargeRewardWithoutGameplay: function (callback, options) {
        Utils.addAdsEvent();
