var Utils = {
    version : 'v2',
    dataVersion : 'v1',
    gameName : 'fishing_league',
    shortName : 'fishing_league',
    generateUsername : function(){
        ...
        var variation0 = [
            'Salty', 'Rusty', 'Silver', 'Golden', 'Tidal', 'Oceanic',
            'Captain', 'Mighty', 'Steely', 'Frosty', 'Deep-Sea', 'Briny',
            'Coastal', 'Reef', 'Wave', 'Current','Hungry', 'Chunky', 'Chubby'
        ];
        var variation1 = [
            'Angler', 'Hook', 'Reel', 'Cast', 'Net', 'Trawler',
            'Mariner', 'Skipper', 'Baiter', 'Lunker', 'Marlin', 'Mako',
            'Tuna', 'Bass', 'Wader', 'Taster'
        ];
    ...
    numberScaleWithoutFloat : function(number){
        if (number >= 1e15) {
            return (number / 1e15).toFixed(0) + 'Q'; // Quadrillions
        } else if (number >= 1e12) {
    ...
    getItemAsArray : function(key, defaultArray){
        if(this.getItem(key)){
  ...
  levenshteinDistance: function(str1, str2) {
    // Normalize inputs - convert to lowercase and remove special characters
  ...
  /**
   * Normalizes text by converting to lowercase and removing common substitution characters
   ...
   * @param {string} str1 - First string
   * @param {string} str2 - Second string
   * @return {number} Similarity ratio between 0 and 1 (1 being identical)
   */
  calculateSimilarity: function(str1, str2) {
    ...
    if (maxLen === 0) return 1; // Both strings are empty or non-alphabetic
    
    const distance = this.levenshteinDistance(normalized1, normalized2);
    return 1 - (distance / maxLen); // Convert to similarity ratio
  },
  
  /**
   * Check if a word contains or is similar to any word in the cuss words list
   * @param {string} word - The word to check
   * @param {Array<string>} cussWords - Array of cuss words to check against
   * @param {number} [threshold=0.75] - Similarity threshold (0-1)
   * @return {boolean} True if the word is similar to a cuss word, false otherwise
   */
  checkSimilarity: function(word, cussWords, threshold = 0.75) {
    if (!word || !cussWords || !Array.isArray(cussWords)) {
      return false;
    }
    ...
    // Check if word contains any cuss word
    for (const cussWord of cussWords) {
      const normalizedCuss = this.normalizeText(cussWord);
      
      // Skip empty cuss words
      if (!normalizedCuss) continue;
      
      // Check if word contains the cuss word
      if (normalizedWord.includes(normalizedCuss)) {
        return true;
      }
      
      // Check similarity for complete words
      const similarity = this.calculateSimilarity(normalizedWord, normalizedCuss);
