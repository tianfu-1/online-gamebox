        ...
        app.on('start', () => {
            ...
            EventTypes.afterAll(EventTypes.LOCATION_CHANGED, EventTypes.LEVEL_CLOTHES_READY).then(() => {
                AssetsLoader.getInstance().loadPendingAssets().then(() => {
                    ...
                    pc.app.fire(EventTypes.AUTO_START_FIRST_LEVEL_AFTER_PRELOADER);
                });
            })
        });
    });
...
/* startup */
EventTypes.AUTO_START_FIRST_LEVEL_AFTER_PRELOADER = 'gameplay:autoStartFirstLevel';
...
EventTypes.ACTIVATE_OPPONENT_FINISHER = 'opponent:activateFinsiher';
...
GameConfig.attributes.add('UI', {
    type: 'json',
    schema: [{
        name: 'autoStartLevelAfterPreloader',
        type: 'boolean',
        default: true
    }, {
...
class ScreenMainMenu extends BaseWindow {
    ...
    initialize() {
        ...
        this.app.on(EventTypes.AUTO_START_FIRST_LEVEL_AFTER_PRELOADER, this.autoStartLevel, this);
    }
    ...
    autoStartLevel() {
        if (GameConfig.getAttribute('UI', 'autoStartLevelAfterPreloader')) {
            UIController.getInstance().hide(Constants.Screens.MAIN_MENU, true);
            this.launchCurrentLevel();
        }
    }
    ...
    updateBoostersVisibility() {
        ...
        this.likesPriceText.element.text = `${Math.floor(likesPrice)}`;
        this.gemsPriceText.element.text = `${Math.floor(gemsPrice)}`;
    ...
    _disableButtons() {
        ...
        this.tapToPlayZone.setAvailable(false);
    }

    _enableButtons() {
        this.buttonShop.setAvailable(true);
        this.tapToPlayZone.setAvailable(true);
    }
}
...
Character.prototype.playFinisherAnim = function (forceFinisherTrigger = undefined) {
    ...
    if (forceFinisherTrigger) {
        ...
        finisherTrigger = Utils.getRandomItem(['bye_sweetie', 'knock_out', 'giant_slap', 'side_kick']);
        this.app.fire(EventTypes.ACTIVATE_OPPONENT_FINISHER, ShopManager.getInstance().getFinisherByKey(finisherTrigger));
    }
...
class ScreenVictory extends BaseWindow {
    ...
    async _processVictoryTweens() {
        ...
        if (lastLevelRewardGems > 0) {
            ...
            await APIMediator.showInterstitialAd("break:result:success");
            ...
            this.gemsAmountText.element.text = `${Math.floor(lastLevelRewardGems)}`;
            await Utils.wait(750);
...
GemCounter.prototype._updateGems = function (value) {
    if (value !== this._gemsValue || !this._gemsValue) {
        ...
        if (this.tweenValues) {
            ...
            this.textGems.element.text = `${Math.floor(value)}`;//`${Utils.formanMoney(value)}`;
        }
    }
};
...
class ScreenDefeat extends BaseWindow {
    ...
    _onShow() {
        ...
        this._processDefeatTweens();


    }

    async _processDefeatTweens() {
        await Utils.wait(1000);
        await APIMediator.showInterstitialAd("break:result:fail");
        await Utils.wait(500);
...
HammerController.prototype._buildCheckpoints = function (numCheckpoints, checkpointValueStep, totalHeight) {
    ...
    for (let i = 0; i < numCheckpoints; i++) {
        ...
        checkpoint.setLocalPosition(-3.8, this.checkpointStartY + i * this.checkpointStep, 0.85);
...
ShopManager.prototype.getFinisherByKey = function (key) {
    return this.finishersCollection.find(x => x.name === key);
};
...
ButtonRandomUnlock.prototype.setPurchasePrice = function(value) {
    this.priceText.element.text = `${Math.floor(value)}`;
};
...
CharacterFinishers.prototype.initialize = function () {
    ...
    if (this.entity.isPlayer()) {
        ...
        this.app.on(EventTypes.ACTIVATE_OPPONENT_FINISHER, this.activateFinisher, this);
    }
};
